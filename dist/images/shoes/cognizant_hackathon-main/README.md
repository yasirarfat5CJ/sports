...
- `src/config.py`: common settings, feature names, file paths
- `src/preprocessing.py`: cleans and validates the dataset
- `src/visualization.py`: creates EDA graphs
- `src/models.py`: builds and tunes ML models
- `src/evaluation.py`: calculates metrics, CV scores, feature importance
- `src/prediction.py`: loads the saved model and predicts marks
- `src/reporting.py`: writes the final report
