...
import { BarChart3, HelpCircle, Trophy, Scale, ShieldAlert, Cpu } from 'lucide-react';
...
export const Analytics = () => {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      setLoading(true);
      const result = await predictionApi.getMetrics();
      if (result.success) {
        setMetrics(result.data);
      } else {
        setErrorMsg('Could not fetch real-time metrics. Displaying pre-cached baseline evaluations.');
      }
      setLoading(false);
    };
    fetchMetrics();
