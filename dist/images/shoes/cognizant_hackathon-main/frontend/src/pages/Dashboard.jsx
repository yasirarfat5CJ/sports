...
import { 
  ArrowRight, Award, ShieldAlert, Activity, ClipboardList, 
  BarChart3, BrainCircuit, ArrowUpRight, History
} from 'lucide-react';
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      if (metricsRes.success) {
        ...
        setErrorMsg('Some dashboard statistics could not be loaded.');
      }
  ...
  // Compute stats based on loaded API responses
  const getSelectedModelMetrics = () => {
    if (!metrics || !metrics.models) {
      return { r2: '0.82', mae: '6.25 marks' }; // Fallback defaults
    }
    
    // Find Gradient Boosting or the chosen model
    const selectedName = metrics.selected_model || 'Gradient Boosting';
    const modelData = metrics.models.find(m => m.model.replace(' Tuned', '') === selectedName.replace(' Tuned', ''));
    
    if (modelData) {
      return {
        r2: parseFloat(modelData.r2).toFixed(3),
        mae: `${parseFloat(modelData.mae).toFixed(2)} marks`
      };
    }
    return { r2: '0.821', mae: '6.25 marks' };
  };

  const modelStats = getSelectedModelMetrics();
  
  // Find the latest prediction in history
  const latestPrediction = history.length > 2 ? history[history.length - 1] : null;
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard
            ...
            value={latestPrediction ? `${latestPrediction.predicted_final_marks} / 100` : '—'}
            icon={Award}
          ...
          <MetricCard
            ...
            trend={latestPrediction ? 'Last Run' : null}
            trendType={latestPrediction?.risk_level === 'Low' ? 'positive' : 'negative'}
          />
          <MetricCard
            title="Predictor Model R²"
            value={loading ? '...' : modelStats.r2}
            icon={Activity}
            description="Explained variance score of the selected model."
            trend="R² Score"
            trendType="positive"
          />
          <MetricCard
            title="Average Error (MAE)"
            value={loading ? '...' : modelStats.mae}
            icon={ClipboardList}
            description="Average prediction deviation on independent test sets."
              ...
              <tbody className="divide-y divide-slate-800/40">
                {history.slice().reverse().map((entry, idx) => (
                  <tr key={idx} className="hover:bg-slate-900/30 transition-colors">
                    <td className="py-3.5 px-4 font-medium text-slate-400">{entry.date}</td>
